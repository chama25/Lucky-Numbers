...
import LuckyNumbers from "../components/LuckyNumbers/index.js";
...
const getSeriesOfLuckyNumbers = (
  ...
  for (let i = 0; i < numSeries; i++) {
    ...
    originNumbersObjArr.forEach((numObj, i) => {
      ...
      originNumbersObjArr[i].numsAfterClicks.push(
        parseInt(numbersArrAfterClick[i])
      );
    });
  }

  // changinNumbers <=> lucky numbers
  let changingNumbers = originNumbersObjArr.filter(
    (numObj) => numObj.isChanging
  );
...
describe("LuckyNumbers Component", () => {
  ...
  test("Clicking 'Show me lucky numbers' creates seven numbers from 1 to 49", () => {
    ...
    const luckyNumbers = getSeriesOfLuckyNumbers(() => {
      userEvent.click(showBtn);
    });
  ...
  test("Last number (super number) is always between 1 and 10", () => {
    ...
    const superNumber = getSeriesOfLuckyNumbers(
      () => {
        userEvent.click(showBtn);
      },
      20,
  ...
  test("'reset' button removes lucky numbers", () => {
    ...
    // find all Elements that contain a number from 1-49
    const numbersElBeforeReset = screen.getAllByText(
      /\b([1-9]|[12][0-9]|3[0-9]|4[0-9])\b/
    );
    userEvent.click(resetBtn);
    const numbersElAfterReset = screen.getAllByText(
      /\b([1-9]|[12][0-9]|3[0-9]|4[0-9])\b/
    );
